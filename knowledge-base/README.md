...
| `The_Mix.html` | The resonance walk — the honest alternative to a typing quiz. Twelve blind forced choices, every card verbatim from `types_data.json` (triggers, mishearings, engines, costs; pattern names masked until the receipt), equal exposure for all nine types, ending in a **reading order** — never a verdict, never a score. Includes the wince rounds and the optional mirror pass (someone who knows you walks the same twelve; divergence is surfaced as the useful page). Nothing stored. See spec §20. |
