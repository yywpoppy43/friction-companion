...
| `The_Field_Kit.html` | The practical layer: brief yourself before a hard conversation (pocket card of tripwires/ways back/pairing rub, expandable to verbatim), steady yourself during (four steps with a breath pacer at the validated dose), walk it back after (ordered recognition prompts). Everyday language by design; reads grid/types/protocol/pairs; fully self-contained. |
| `The_Mix.html` | The resonance walk, first build (spec §20) — superseded by The Mix II after the owner's voice ruling; kept per the versioning rule. |
| `The_Mix_II.html` | The resonance walk, second build (spec §22): same architecture — twelve blind forced choices, equal exposure, the wince rounds, the optional mirror pass, a **reading order** never a verdict, nothing stored — with every card now a short restatement of exactly one authored line (trigger/filter/engine/cost), in first person for the walker and third person for the mirror pass. The receipt names the source line behind every choice. |
| `Source_Ledger.md` | The owner's attribution record: which lineage (Ichazo / Naranjo / Riso-Hudson / Horney) each field descends from, what is genuinely ours, known gaps, the copyright rule. Attribution lives here so it never leaks into any product's output. Future data face for the Composition Engine's sources panel. |
...
| `pairs_data.json` | Validated research face ([STRUCTURE]): all 45 type combinations. Riso-Hudson/Enneagram Institute fields — what each brings, where it rubs; `repair` empty because that source has none. Plus `repair_daniels` — the David Daniels second school (prompt-6 research, validated 45/45, no ranking language), displayed in Guide III as a separately-labeled "What helps — a second school" block, never blended. |
